...
'Five': ['challenge', 'disruption'], 'Six': ['harmony', 'exchange'], 'Seven': ['assessment', 'perseverance'], 'Eight': ['movement', 'mast
ery'],
...
if choice == '':
raise RuntimeError('No model selection was entered. Run ./create_data.sh in a terminal and select a numbered model, or set OLLAMA
_MODEL for automation.')
choice = choice.strip()
if choice.isdigit():
index = int(choice)
...
raise ValueError(f'Model response for {card["name"]} did not include two usable upright and two usable reversed sentences. Response p
review: {preview}')
...
print(f'Confirm Ollama is running and the selected model is available, e.g. `ollama pull llama3.1` or run `./create_data.sh --list-models
`.', file=sys.stderr)
sys.exit(1)
PYTHON
